...
- **The application exists.** 14 modules under `crm/app/Modules/`, 31 dated migrations (34 files with Laravel's three framework ones), and a Vue 3 + TypeScript SPA on `/api/v1` (`D-67`). Measured on `fix/supplier-quotations-currency-reachable` (fix-pass F-01, 2026-09-16): **2861 backend tests (18064 assertions)** and **857 frontend tests (49 files)**. Stack is **Laravel**, recorded as **D-57** in the decision log and in §14.2.
