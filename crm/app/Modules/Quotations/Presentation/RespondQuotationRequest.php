 ...
 * is refused, not dropped (A); `accepted` and `rejected` wait for 1.6 and 1.5
 * (B). A counter's missing reason is `RespondToQuotation`'s to refuse, because
...
final class RespondQuotationRequest extends FormRequest
{
    ...
    {
        return [
            'response' => ['required', 'string', Rule::in(['partial', 'counter'])],
            'reason' => ['nullable', 'string', 'max:2000', 'prohibited_unless:response,counter'],
