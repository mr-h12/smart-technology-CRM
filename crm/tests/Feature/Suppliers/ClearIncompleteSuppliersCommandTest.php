 ...
 */
final class ClearIncompleteSuppliersCommandTest extends TestCase
{
    use RefreshDatabase;

    private const COMMAND = 'suppliers:clear-incomplete';

    public function test_that_a_complete_flagged_row_is_cleared_and_audited_by_the_system(): void
    {
        $id = $this->flagged(['name' => 'Gap Supply', 'type' => 'supplier', 'phone' => '0100', 'contact_person' => 'Mona']);

        $this->run_()->expectsOutputToContain('1 of 1')->assertExitCode(0);

        $this->assertDatabaseHas('suppliers', ['id' => $id, 'is_incomplete' => false, 'updated_by' => null]);
