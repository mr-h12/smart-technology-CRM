...
| D-85 | **Suppliers are imported from CSV like customers, and an imported supplier with missing fields is flagged incomplete** (recorded 2026-09-21, proposed — awaiting the owner's approval at merge; the owner's F-09 request). `D-31` accepts incomplete data on import and §3.3 gives customers an import; suppliers had none, and `suppliers` had no `is_incomplete`. `POST /suppliers/import` reads CSV through `fgetcsv` with no library (the owner's 2026-08-29 ruling for customers). It is guarded by a new `catalog.import` permission in §3.7, granted to the Manager alone as §3.3's customer import is, although `D-45` opens single edits to every operational role. The file may carry `name`, `type`, `phone`, `contact_person` and `has_open_account`. `color_rating` is not imported: `D-19` makes it a manual rating, and §7.1's White ("new / not yet rated") is what an imported supplier is. A row with no `name`, or with a `type` that §7.1 does not name (supplier / distributor — the same closed set the form and a database CHECK enforce), is counted and not saved (owner's ruling, 2026-09-21). A row with an empty `type`, `phone` or `contact_person` saves with `is_incomplete = true`. An empty `has_open_account` is read as no. The whole file runs in one transaction, with an audit row per supplier and one `supplier_import_batches` row owned by the Suppliers module; modules do not share tables, so this is not the Customers module's `import_batches`. The format rules that `CustomerCsv` already implements (BOM, `;` separator, CRLF) move to one shared reader instead of being copied. `is_incomplete` is set by the importer only: it is in the supplier payload and in `filter[is_incomplete]`, and a write that sends it is a 422. `is_active` stays the supplier's only lifecycle flag; there is no `is_archived`. **Not covered:** the flag is never cleared once set, for suppliers as for customers, although §11 says "until completed" (one debt row for both); no duplicate detection on import; no `.xlsx`; the supplier-quotations screen's capped supplier list (existing debt), which an import now makes reachable; catalog items and the product↔supplier link (`D-86`, F-10). |
| D-87 | **An imported record's "incomplete" flag clears when an edit completes it, and a customer is complete when its core fields are filled** (recorded 2026-09-21, proposed — awaiting the owner's approval at merge; the owner's F-11 request). `D-31` flags an imported record with missing fields and §11 excludes it from financial reports "until completed", but only the importer wrote `is_incomplete` and no edit cleared it (`D-85` "Not covered"). A **supplier** is complete when `type`, `phone` and `contact_person` are filled (`D-85`'s rule). A **customer** is complete when `name`, `sector`, `region`, `contact_person` and `phone` are filled; the customers' importer narrows to the same five fields, replacing its undocumented any-of-ten reading, so import and edit agree (owner's ruling, 2026-09-21). **Clear only:** an edit that leaves every core field filled sets `is_incomplete = false`; an edit that empties one never sets it, because `D-31` makes the flag the importer's and a hand-typed record is never incomplete. The change is recorded in the same `SUPPLIER_UPDATED` / `CUSTOMER_UPDATED` audit row as the edit (`AUD-02`, old and new). The field stays prohibited in a write request. A **one-off, idempotent correction** per module clears the flag on existing records that are already complete, one audit row each. **Not covered:** re-flagging; duplicate detection; the Arabic-Indic dates (F-12, not ordered). `D-86` is reserved for F-10. |
| D-86 | **Catalog items are imported from one CSV, an imported item missing what the form requires is flagged incomplete, and a catalog item records which suppliers carry it** (recorded 2026-09-21, proposed — awaiting the owner's approval at merge; the owner's F-10 request, rulings Q0–Q9 and A1–A4). `D-31` accepts incomplete data on import; §7.3 keeps the catalog "descriptive data only — no prices". `POST /catalog-items/import` reads one CSV through the shared `CsvReader` with the columns `kind`, `product_code`, `name`, `category`, `unit`, `service_type`, `description`, `company`, `notes`, `is_active`, `supplier`; an empty `is_active` means active. It is guarded by `catalog.import`, seeded to the Manager and granted or revoked by the administrator through the permission matrix (§3.12 rule 5) — the Manager is a default, not a rule. **Rejected and counted, not saved:** an unknown or missing `kind`; a product with no `name`; a `unit`, `service_type` or `company` not in its managed list (the import never adds a list value, although the form adds an unknown company); a value longer than its column; a `supplier` matching no supplier or more than one. **Saved with `is_incomplete = true`:** a product with no `unit`, a service with no `service_type`, any row with no `company`. Values match a list entry's code or either label, and a supplier's name, after trimming and ignoring case; active and deactivated suppliers both match; a blank `supplier` saves the item unlinked. Every row creates a new item. A new **item↔supplier link** (`catalog_item_suppliers`, many suppliers per item, no price or quantity) is written by the import (zero or one per row) and edited by hand on the item under `catalog.manage`, audited old and new. An edit that completes a flagged item clears the flag in the same `CATALOG_ITEM_UPDATED` audit row, clear-only (`D-87`'s shape); `is_incomplete` is in the payload and `filter[is_incomplete]`, and a write that sends it is a 422. The whole file runs in one transaction, with an audit row per item and per link and one `catalog_import_batches` row owned by Catalog. Supplier names are resolved through a lookup the Suppliers module publishes, never its tables. **Not covered:** duplicate detection; per-row rejection reasons; `.xlsx`; supplier prices or quantities on the link; re-flagging; the form's acceptance of off-list units and service types (debt). |
| D-88 | **Supplier offers are searched by their deal's code, and show it** (recorded 2026-09-22, proposed — awaiting the owner's approval at merge; the owner's F-13 request). `GET /supplier-quotations` gains `filter[deal_code]`: a fragment of a deal's code, trimmed and case-insensitive, matching every offer whose deal's code contains it; an offer with no deal never matches. It passes through `SearchService` like every search, so `code` joins the deals index, and the deals list's own `q` finds a deal by its code too (the owner accepted that side effect). Every offer carries `deal_code` beside `deal_id`, null when it has no deal. SupplierQuotations reads codes through a contract Deals publishes (`DealFactsInterface`), never the `deals` table. `filter[deal_id]` stays. **Not covered:** the offer form's deal field, which still takes a UUID (debt); a deal picker; searching offers by a deal's title or customer. |
| D-89 | **The customer quotation PDF follows the company's own offer form, superseding `D-79`'s P-01 layout; the full totals block stays** (recorded 2026-09-22, proposed — awaiting the owner's approval at merge; the owner's ruling on the form supplied that day). **Source:** `docs/reference/offer-form-2026-09-22/` — `new-offer-form.docx` (the company's English offer form) and its letterhead logo and footer. **What is approved — the layout:** the letterhead logo top-left with the faded `S.T.I.S` mark behind the page; a header block of **Date · To · Att · Subject**; the opening line *"We have the pleasure to provide you with the following offer"*; an item table **S · Item · Unit Price · Qty. · Total price · Delivery time**; a bulleted conditions list; the closing *"Please do not hesitate to contact us in case you have any questions"* and *"Best Regards,"* over **one signatory** — no "Prepared By / Approved By" pair, because approval is internal (Module 8) and not the customer's business; and the footer band with address, phones, fax and e-mails. **What the owner kept from the system rather than from the form:** the **full totals block** under the table — subtotal, discount, tax, additional items, rounding and final total exactly as §5 computes them, discount before tax (`D-64`) and **no tax row at all on an exempt quotation** (`D-63`). The form's bullet *"Price includes value add tax"* is therefore **not printed**: next to a separate tax row it would be false, and on an exempt quotation doubly so. **Carried over from `D-79` unchanged:** one template renders Arabic (RTL) and English (LTR) with no duplicated markup and no hard-coded user-facing strings — the form is English only, so the Arabic page is its mirror, not a second design; fonts embedded as base64; rendering through Browsershot on the `pdf` queue (`D-57`); live page numbering; and the customer-view model that structurally cannot hold supplier, cost or margin fields (Module 9 Point 1.1–1.2, #118/#201). `D-79`'s one-page guard becomes a page-break guard: the form is a letter whose table may run onto a second page, and a row must never split across one. **Where each form field comes from — named defaults, each owned by the module that holds the data:** *To* — the customer's name (`CustomerNamesInterface`, `D-83`). *Att* — the customer's single `contact_person` (`D-18`); omitted when blank. Customers publishes it (Module 3 — requested from its owner). *Subject* — the deal's `title` (Module 5); the line is omitted when the deal has none, never invented. *Delivery time* per line — **a new nullable free-text field on the quotation line** ("2 - 4 Weeks"), a column on `quotation_items` and therefore Module 7's to add (requested); until it exists the column is **not printed**, and the quotation-level `delivery_terms` still print as a condition when `show_delivery_terms` is true. *Offer currency · Payment · Warranty* — the quotation's currency name, `payment_terms` and `warranty`, one bullet each, omitted when blank. *Offer validity* — `valid_until`, printed as a date; the form's free text ("2 B. Days or stock last") is not a stored value and is not reproduced. *Delivery site* and *"The company operates under an advance payment system"* — company-wide texts, so **new Settings keys** beside `CompanyName`/`CompanyAddress`/`CompanyPhones` (Module 2 — requested), each omitted until set; likewise the **company e-mails and fax** the footer shows. *Signatory* — the quotation's creator, named through Identity's `UserFactsInterface`. The form's job title ("Sales Manager") is **not stored anywhere**: users have no title field, so the title line is omitted until Identity (Module 1) holds one — requested, not substituted with a role name the person may not use on paper. **Not settled here:** the Arabic wording of the opening and closing lines, which the owner writes rather than a translation tool; and every omitted-until-requested field above, which is a gap on the page and not a blank label. |
| D-90 | **A quotation is sent without waiting for its PDF, and the customer's response moves the deal on two events only** (recorded 2026-09-23, proposed — awaiting the owner's approval at merge; the owner's Module 10 answers Q1, Q2, Q6, Q8 and Q12 and edge rules a–b, approved with the point list in #206). **Send without the PDF:** Flow 1 step 8 and §6.1 ("Sent — PDF generated") couple sending with the PDF; Module 10 builds `approved → sent` (`PATCH /quotations/{id}/send`) without requiring one, because PDF generation is Module 9's and still in progress — connecting the two is Module 9's to do. **Two deal moves:** sending moves the deal `supplier_quotation → quotation_sent` (§4.4 "Quotation Sent · Sales (after approval)"); a deal before `supplier_quotation` refuses the send (`422 deal_not_ready_to_send`), and a deal at `quotation_sent` or later is left where it is (rule a). A rejection — `sent → rejected`, or `expired → rejected` as §10.5 allows — moves the deal to `lost`, the rejection reason becoming its lost reason, **only when no other quotation of that deal is live** (`draft`, `pending`, `approved`, `sent`): a deal may hold several, so the count is taken under a row lock inside the rejection's transaction. A deal with no `lost` edge (already `lost`, or `won` and beyond) is left untouched and the response says so (rule b). Accepted, Partial and Counter never move the deal — §4.4 gives `won` to TL / Manager. **No revival:** a rejected quotation is archived in the list's `history` bucket and is never restored; Flow 7's "Restore · Manager" does not reach it, because the deal it closed is terminal (§4.4). When the customer comes back, a new deal is opened — a `lost` deal is never revived |
| D-91 | **The Team Leader and Procurement read every quotation** (recorded 2026-09-23, the owner's ruling during Module 10 · 2.2's questions; built in 2.2a). §3.5's `view` row gave the Team Leader `Team` and Procurement `Asgn`; both read **All**, and through the quotation every purchase order (Module 10 Q10: a PO is read by whoever may view its quotation). Their bare ✅ cells in §3.5 — the Team Leader's view cost & margin, edit margin, edit tax and export/download PDF, and Procurement's view cost & margin — follow the new cell, as §3.2 reads every bare ✅; so both see cost and margin on every quotation, which the owner accepted. **Every explicit cell is unchanged:** the Team Leader keeps `Team` on create, edit, submit, approve, return, send, generate PDF, record response and delete (still unbacked, `D-a`); Procurement keeps `Asgn` on generate and export PDF. A database seeded before this row is corrected by the migration `2026_09_23_100000_grant_quotation_view_all_to_team_leader_and_procurement`, audited as `ROLE_PERMISSIONS_UPDATED` with the system actor |
...
| import (CSV) | Manager by default — the administrator grants or revokes it (§3.12 rule 5, `D-85`, `D-86`) | — |
...
| ~~OD-02~~ ✅ | ~~PDF template~~ — **closed by `D-79`** (2026-09-13), **layout superseded by `D-89`** (2026-09-22, the company's own offer form): the `P-01` prototype template was the approved design, with its three carried-forward items named there as Module 9 build work | — | — |
